...
class PackedSampleDataset(Dataset):
    """
    每个样本在一个目录：包含 20 张图片(按名字排序) + label.txt(0/1)
    返回: (x: [20,3,128,128], y: LongTensor[ ]) 
    """
    def __init__(self, root_dir, split="train", transform=None):
        super().__init__()
        self.root = os.path.join(root_dir, split)
        self.sample_dirs = sorted([d for d in glob.glob(os.path.join(self.root, "*")) if os.path.isdir(d)])
        self.transform = transform or img_tf_eval

    def __len__(self):
        return len(self.sample_dirs)

    def __getitem__(self, idx):
        sdir = self.sample_dirs[idx]
        # 读 20 张图（按文件名排序）
        img_paths = sorted(glob.glob(os.path.join(sdir, "*.png"))) + \
                    sorted(glob.glob(os.path.join(sdir, "*.jpg"))) + \
                    sorted(glob.glob(os.path.join(sdir, "*.jpeg")))
        # 只取前 20 张（或断言必须是 20）
        assert len(img_paths) >= 20, f"{sdir} has {len(img_paths)} images < 20"
        ...
        for p in img_paths:
            ...
            imgs.append(im)
        x = torch.stack(imgs, dim=0)                   # [20,3,128,128]

        # 读标签
        with open(os.path.join(sdir, "label.txt"), "r") as f:
            y = int(f.readline().strip())
        y = torch.tensor(y, dtype=torch.long)          # CE 需要 long

        return x, y
    
train_set = PackedSampleDataset("dataset", split="train", transform=img_tf_train)
test_set  = PackedSampleDataset("dataset", split="test",  transform=img_tf_eval)
...
class MultiHeadAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        seq_w   = 16     # 128 // 8
        d_half  = head_dim // 2
        assert dim % self.num_heads == 0 and (dim // self.num_heads) == head_dim
        assert d_half % 2 == 0, "对称 2D RoPE 需要 head_dim/2 为偶数"
...
model = WriterMatcherCE(
    ...
    dim=384, depth=6, heads=6, mlp_dim=768,
    dropout=0.1, emb_dropout=0.1, pool='mean'   # 或 'attn'
).to(device)

criterion = nn.CrossEntropyLoss()  # 你的 loop 用的就是 CE 风格
...
for epoch in range(1, num_epochs + 1):
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(model.state_dict(), "best_writer_matcher.pt")
